...
class ForgingblockAPI{
	...
	public function CoinFullName(){
	  return $this->StatusData['paymentMethodName'];
	}
	
	public function coinLogo(){
	  return $this->APIBaseURL.$this->StatusData['cryptoImage'];
	}	
	
	public function GetStatus($invoice_id, $coin) {
		$options = array(
			'timeout' => 300,
		);	
		
		$url = $this->APIBaseURL.'/i/'.$invoice_id.'/'.$coin.'/status?invoiceId='.$invoice_id.'&paymentMethodId='.$coin.'&_=1';
		...
		$resar = Requests::get($url, $headers);						
		
		$this->StatusData = json_decode($resar->body, true);		
